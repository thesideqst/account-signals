...
**Phase 1 (text/turns only) shipped 2026-09-09** — see the dated entries at
the end of the Decisions log. The model writes HOST_A/HOST_B turns instead of
a monologue, and `gold_briefing` carries them as `script_turns` alongside the
existing `script_text`. **Per-voice TTS is deliberately still not done** — see
"What it changes" below; `tts.py` keeps reading the single concatenated
`script_text` exactly as before. That is phase 2, left in this Backlog entry
on purpose rather than deleted, so it stays visible as unscheduled work.
...
- 2026-09-01 — A used topic is recorded in Unity Catalog (`gold_topic_usage`), not marked in Postgres. Federation reads Postgres but cannot write to it, and psycopg inside a serverless task kills the Python kernel, so the briefing job has no way to mark a topic used at the source. It writes to UC — the direction that works — and `topic_queue_current` subtracts it. Before this the scheduled path read `ORDER BY requested_at ASC LIMIT 1` and got request 1 on every run forever, however many episodes had already covered it
- 2026-09-01 — Usage is recorded AFTER the episode publishes, never before, so a run that dies leaves the topic outstanding rather than silently consumed
- 2026-09-01 — `generating` is no longer a terminal state. The app set it when a rep triggered a run and nothing ever moved it on, so a failed or cancelled run stranded that topic outside the queue permanently. A run takes two to four minutes, so anything still generating after thirty is a run that died and the topic returns. The same rule is applied in the view and in `/api/topics`, so the tab and the pipeline agree on what is outstanding
- 2026-09-01 — Mode C falls back to a STANDING SUBJECT when no rep has asked and the queue is empty. Previously that produced an episode with no subject at all, which is the single case where the model would have had to choose from its own knowledge — the exact hole the grounding rule exists to close. Nine subjects, supplied by the rep: how the business makes money, company history, who the executives are, how they are paid, supply chain, big bets, competitors, SWOT, technology partnerships
- 2026-09-01 — The subject is chosen DETERMINISTICALLY, not by the model: unused subjects in list order, and among those the first with at least four matching chunks for that account. Letting the model choose would put the subject and the script in the same hands, which is the same objection that keeps mode selection in SQL — a wrong subject should be a query to inspect, not a generation to re-run
- 2026-09-01 — Each standing subject carries its own MATCH TERMS rather than relying on splitting the phrase into words longer than four characters. "big bets" has no such word, so the generic splitter would produce an empty filter and the query would return whatever was most recent — which is precisely how a McKinsey piece on Moderna once became an NVIDIA source
- 2026-09-01 — The prompt does not claim the rep asked for a subject the system chose. It says plainly that no one asked and that this is a standing subject, and forbids implying otherwise. A briefing whose whole claim is that it does not invent things cannot open by inventing a request. Verified: zero "you asked" claims in the generated script
- 2026-09-01 — Topic usage is written as a DataFrame, never interpolated into a SQL string. Doubling the quote in "the company's history" did not survive the round trip — the row came back as "the companys history", 40 characters against 41 — and because the rotation matches the STORED topic against the list, that subject would never match and would be chosen again on every quiet day, which is the exact repetition the feature exists to prevent. Caught only because the stored value was read back and its length checked, not because anything failed
...
- 2026-09-01 — The write-back sync selects what is MISSING by id (a left anti join), not `id > max(id)`. Postgres hands out sequence values at INSERT, not at COMMIT, so two concurrent writes can become visible out of order: the transaction holding id 5 can commit after the one holding id 6. A high-water mark reading in that window sets hwm to 6 and never looks back, so row 5 is lost permanently — no error, no gap check, and the recap simply never reaches grading. An anti-join makes that impossible rather than merely detectable, and still cannot double-count
- 2026-09-01 — A row found missing from BELOW the high-water mark is reported as LATE ARRIVAL rather than silently repaired, because a silent repair hides how often the race actually happens. Proved by deleting answer 5 from the Unity Catalog copy and re-running: the warning fired, the row came back alongside the new one in a single pass, and a second run was a clean no-op at 11 rows and 11 distinct ids
- 2026-09-01 — The tables hold tens of rows, so a full anti-join is cheap. If they grow, bound the scan by `created_at` rather than returning to a watermark — the watermark is the thing that was unsafe, not the scan size
- 2026-09-01 — A missing ingest timestamp stays NULL in the lineage JSON instead of becoming the string "None". `str(None)` produced a truthy value that sorts above every digit, so the UI picked it as the freshest of six timestamps and showed "data unknown" while five real times sat beside it. A REGRESSION FROM THE SAME DAY: it only appeared once provenance counts became account-scoped, because a source with no rows for that account — GOOG and MU have no analyst ratings at all — returns NULL where a table-wide count never did. Scoping the counts was right; carrying the NULL through as a string was not
- 2026-09-01 — The UI also filters the freshness list to values that actually parse as dates, so a non-date can never win the sort again. Fixed at both ends deliberately: the source fix stops new episodes carrying it, and the client guard repairs every episode already published without regenerating any of them — verified on MU, which still holds the old "None" and now reads "17h ago"
- 2026-09-01 — A repeat submission inside 60 seconds returns the FIRST result rather than creating a second row. A duplicate was never a harmless extra: recap 3 and 4 are byte-identical 10.06 seconds apart and were graded separately into DIFFERENT verdicts — accuracy 35 and 30, six gaps and five — so a double-tap forked the record of what the rep understood, and `recap_id DESC` then made the callback prefer the later, lower one. The window sits between the observed 10-second double-tap and a real 16-minute retry
- 2026-09-01 — The answer guard runs after transcription but BEFORE grading, so a duplicate costs no LLM call as well as no row. It cannot run before transcription, because the transcript is the thing being compared
- 2026-09-01 — Company news now comes from SEC 8-K filings and their press-release exhibits, not only from headline feeds. Measured across `silver_doc_chunks`: Google News and Yahoo average 186 characters and top out at 620, while the industry feeds average 2,649 and transcripts 1,081. A 186-character stub is a headline plus a truncated teaser, and every fabricated figure the audit found sat on one. The grounding rule cannot hold when the grounding material is a headline — the model is handed a title and asked to narrate its significance, which is a request to speculate. Three accounts yield 311,503 characters of full text
- 2026-09-01 — Filings were chosen over scraping publisher article bodies. Fetching an article's text is a different act from reading the summary a publisher puts in a feed, and this project already ruled Seeking Alpha out on terms-of-service grounds when it went looking for transcripts; that reasoning has to extend here rather than be dropped because the data would be useful. SEC filings are public domain, are the company's own words filed under legal liability, and the fair-access policy asks only for identification and a rate limit — both of which `edgar.py` already honours
- 2026-09-01 — Only the 8-K cover page and EX-99 exhibits are kept. Exhibit type is the filter and it has to be: Alphabet's 2026-08-10 bond offering attached ELEVEN "GLOBAL SECURITY" exhibits, EX-4.2 through EX-4.11, at 26,000 to 38,000 characters each — roughly 300,000 characters of indenture boilerplate from one filing, which would swamp retrieval for that account and teach a rep nothing. EX-99 is the press-release class; EX-4 is securities, EX-5 legal opinions, EX-10 contracts — all real documents, none of them news
- 2026-09-01 — Exhibit type comes from the filing's index PAGE, not from `index.json`. The JSON carries a `type` field, but it is the name of the icon EDGAR draws beside the row — "text.gif" for every document — so it cannot be used to tell a press release from a bond indenture
- 2026-09-01 — EDGAR's own machinery is excluded by name AND by content: `R\d+.htm` XBRL viewer renderings, the `<accession>-index.htm` page, and the raw submission header are all HTML, so no extension filter catches them, and each one reads like content to a model that was handed it. The exhibit header line ("EX-99.1 5 d83560dex991.htm EX-99.1 Exhibit 99.1") is stripped token by token from the front rather than with one pattern, because its length and order vary by filer
- 2026-09-01 — An 8-K item code is carried into the chunk as WORDS ("results of operations and financial condition", "a change of directors or principal officers") rather than as "2.02" or "5.02". Same rule as stating a direction in words instead of a signed number: never ask the model to decode a code that matters
- 2026-09-09 — Full-article extraction built for the backlog item "Full-article news, not headlines and teasers", and left DISABLED. New `src/ingest/news_extract.py` fetches an RSS item's article URL and extracts body text (`trafilatura`, added to `resources/ingest.job.yml`'s `ingest_env`) only for a publisher named in that module's `ALLOWED_PUBLISHERS` set — which is, and stays, empty. Populating it for any one publisher is a per-publisher ToS decision for a human, the same reasoning that excluded Seeking Alpha and that `filings.py`'s own docstring gives for choosing SEC filings over scraping when it solved the adjacent half of this problem. This does not duplicate or replace that decision: `filings.py` covers a company's OWN press releases (public domain, filed under legal liability); this module is for third-party journalism and analysis, which still arrives as an RSS stub either way and is not something a company's own filings will ever carry. With the allowlist empty, ingest, chunking and the briefing prompt are byte-for-byte unchanged from before this landed
- 2026-09-09 — Paywall/consent-wall detection (`news_extract.looks_like_paywall_or_stub`) is two checks, either one disqualifying: extracted text under 500 characters (`MIN_BODY_CHARS` — short enough that it is not meaningfully better than the summary already on the row), or a boilerplate phrase match (cookie banners, subscribe walls, bot checks) while still under 2,000 characters (`PAYWALL_PHRASE_SAFE_CHARS` — long enough that one incidental phrase inside a genuinely long article stops being conclusive). Deliberately conservative in the direction of rejecting: a false "success" here would relabel a paywall's own boilerplate `KIND: ARTICLE` and let the model elaborate on it, which is the exact stub-masquerading-as-substance failure this feature exists to remove, moved one layer down rather than fixed
- 2026-09-09 — `chunk_and_embed.py`'s news branch now checks for `body` per row rather than always taking the one-chunk-per-item path: a row with no body (every row today) keeps exactly today's behaviour, and a row with one is packed with the same sentence-boundary mechanism already proven for transcript turns and filing exhibits, title glued to the front of the body as the anchor of the first part, rather than inventing a third chunking scheme. `synthesize.py`'s `STUB_CHARS`/`KIND:` labelling needed no change — it already keys off `chunk_text` length, so a real multi-part article naturally reads as `ARTICLE`
- 2026-09-09 — `bronze_write` (`src/ingest/_common.py`) now writes with `mergeSchema=true`. `news.py`'s `SCHEMA` gained a nullable `body` column and the existing `bronze_news` table predates it; Delta only allows a merge to widen a schema by adding columns, never to drop or retype one, so this can't silently paper over a real schema break in another source
- 2026-09-09 — Two-host dialogue replaces the single-narrator script (Backlog: "Two-host conversation instead of a monologue"), scoped to phase 1: text and turns only. The primary generation call now returns a JSON array of `{speaker, text}` turns instead of prose, parsed the same slice-between-brackets way `EPISODE_META_PROMPT` and `QUESTIONS_PROMPT` already are; `gold_briefing` gains `script_turns` (the parsed array, for a future per-turn audio pipeline) via `ALTER TABLE ... ADD COLUMNS` plus `mergeSchema` on the write as a second safety net, so existing rows get NULL rather than a backfilled guess at a conversation that was never generated — there is no source to reconstruct one from. `script_text` keeps being the concatenated turn text, unchanged in shape, so `tts.py`, grading, and the meta/questions calls need no changes. Per-voice TTS is deliberately deferred to phase 2, not forgotten
